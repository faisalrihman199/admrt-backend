        ...
        if period == 'monthly':
            ...
            for week_number, (start_of_week, end_of_week) in enumerate(weeks_data, start=1):
                ...
                for role in roles:
                    ...
                    if role == 'advertiser':
                        advertisers_accounts[f"Week {week_number}"] += role_count
                    else:
                        ad_hosts_accounts[f"Week {week_number}"] += role_count
...
class MessageCountView(generics.RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if period == 'monthly':
            ...
            weeks_data = []
            for week in range(4):
                start_of_week = current_time - timezone.timedelta(weeks=week + 1, days=current_time.weekday())
                end_of_week = start_of_week + timezone.timedelta(days=6)
                weeks_data.append((start_of_week, end_of_week))

            for week_number, (start_of_week, end_of_week) in enumerate(weeks_data, start=1):
                total_weekly_data = Message.objects.filter(created_at__range=[start_of_week, end_of_week]).count()
                total_messages[f"Week {week_number}"] = total_weekly_data
        ...
        # Process role-specific data for each period
        for role in roles:
            role_id = settings.K_ADVERTISER_ID if role == 'advertiser' else settings.K_SPACE_HOST_ID
            role_queryset = Message.objects.filter(sender__user_role=role_id)

            if period == 'monthly':
                monthly_data = role_queryset.annotate(month=ExtractMonth('created_at')).values('month').annotate(count=Count('id'))
                for entry in monthly_data:
                    month_number = entry['month']
                    month_name = timezone.datetime(current_time.year, month_number, 1).strftime('%b')
                    if role == 'advertiser':
                        advertiser_messages[month_name] += entry['count']
                    else:
                        space_host_messages[month_name] += entry['count']

            elif period == 'daily':
                daily_data = role_queryset.filter(created_at__range=[start_of_week, end_of_week]).annotate(day=ExtractDay('created_at')).values('day').annotate(count=Count('id'))
                for entry in daily_data:
                    day_number = entry['day']
                    day_name = timezone.datetime(current_time.year, current_time.month, day_number).strftime('%a')
                    if role == 'advertiser':
                        advertiser_messages[day_name] += entry['count']
                    else:
                        space_host_messages[day_name] += entry['count']

            elif period == 'weekly':
                for week_number, (start_of_week, end_of_week) in enumerate(weeks_data, start=1):
                    weekly_data = role_queryset.filter(created_at__range=[start_of_week, end_of_week]).count()
                    if role == 'advertiser':
                        advertiser_messages[f"Week {week_number}"] += weekly_data
                    else:
                        space_host_messages[f"Week {week_number}"] += weekly_data

            elif period == 'all':
                for year in range(current_year - 10, current_year + 1):
                    year_start = timezone.datetime(year, 1, 1)
                    year_end = timezone.datetime(year + 1, 1, 1)
                    role_count = role_queryset.filter(created_at__range=[year_start, year_end]).count()
                    if role == 'advertiser':
                        advertiser_messages[year] = role_count
                    else:
                        space_host_messages[year] = role_count
        ...
        if period == 'daily':
            ...
            data = {
                ...
                "totalMessages": [{"name": day, "value": total_messages.get(day, 0)} for day in days],
                "advertiserMessages": [{"name": day, "value": advertiser_messages.get(day, 0)} for day in days],
                "spaceHostMessages": [{"name": day, "value": space_host_messages.get(day, 0)} for day in days],
            }
        elif period == 'weekly':
            data = {
                "totalMessageNums": total_message_nums,
                "totalAdvertiserMessages": total_advertiser_msgs,
                "totalSpaceHostMessages": total_space_host_msgs,
                "totalMessages": [{"name": f"Week {i+1}", "value": total_messages.get(f"Week {i+1}", 0)} for i in range(4)],
                "advertiserMessages": [{"name": f"Week {i+1}", "value": advertiser_messages.get(f"Week {i+1}", 0)} for i in range(4)],
                "spaceHostMessages": [{"name": f"Week {i+1}", "value": space_host_messages.get(f"Week {i+1}", 0)} for i in range(4)],
            }
        elif period == 'all':
            data = {
                "totalMessageNums": total_message_nums,
...
class VisitorCountView(generics.RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if period == 'daily':
            ...
            response_data.update({
                "siteVisitors": [{"name": f"Week {i + 1}", "value": total_visitors.get(f"Week {i + 1}", 0)} for i in range(4)],
                "affiliateVisitors": [{"name": f"Week {i + 1}", "value": affiliate_visitors.get(f"Week {i + 1}", 0)} for i in range(4)],
                "combinedVisitors": [
                    {
                        "name": f"Week {i + 1}",
                        "value": total_visitors.get(f"Week {i + 1}", 0) + affiliate_visitors.get(f"Week {i + 1}", 0)
                    } for i in range(4)
                ],
            })
